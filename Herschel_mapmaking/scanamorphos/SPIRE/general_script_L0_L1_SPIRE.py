...
dir_root = "/pcdisk/stark/aribas/Desktop/modeling_TDs/remaps_Cha/SPIRE/scanamorphos/"
path = dir_root+"L1/"

### number of nominal + parallel-mode observations:
n_obs = 2

use_calib_tree = "spire_cal_12_3"


#######################################################

## Do a multiple target search in the archive and use the "save all results as CSV" option.
## --> ascii tables 'results_nominal.csv' and 'results_parallel.csv' where lines
## can be edited (suppress unwanted observations and correct target names)

## Create the directories contained in the path_processed variables (l. 121)
...
## parameters that are automatically set up by this script:

## nrep: number of "repetitions"
## For an observation consisting of a single scan, nrep = 0 .
## Otherwise, nrep = number of {scan + cross-scan} pairs (total number of scans = 2 * nrep).

## first_scan_num: The first scan of the observation will be given this index
## when saved to disk. In the general case, first_scan_num = 1 .
## For parallel-mode observations where two scans are contained in different obsids,
## first_scan_num = 2 for the second obsid.
...
#table_obs = asciiTableReader(file=dir_root+'results_nominal.csv', tableType='CSV', skipRows=1)
#list_obsids = table_obs[0].data
#list_names = table_obs[1].data

# We only have parallel observations, so modify the script accordingly
### parallel-mode observations:
table_obs = asciiTableReader(file=dir_root+'results.csv', tableType='CSV', skipRows=1)
#list_obsids.append(table_obs[0].data)
#list_names.append(table_obs[1].data)
list_obsids = table_obs[0].data
list_names = table_obs[1].data
...
for i_obs in range(n_obs):
	...
	##
	path_processed = path+source+"_processed_subscans/"
    # create directory if it does not exist
	if not(os.path.exists(path_processed)):
		os.system('mkdir '+path_processed)
	source_prev = source
	vscan_prev = vscan
	##
	##
