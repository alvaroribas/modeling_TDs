### Main script for the MCMC fitting of TDs
### Note only TDs can be fitted with this script, and pre-TDs require a different script.
...
## Here I'll have to find a way to import all the useless code I've written for this
#####

# This is the object that we will be fitting, and some fitting parameters
obj = 'T25'
n_walkers = 50
n_generations = 1000
kind = 'midIR' # also could be 'Herschel' or 'complete'


# path informations
path_main = '/Users/alvaro/Desktop/papers/modeling_TDs/objects/'
path_object = path_main + obj + '/' + obj +'_data/'


# read the photometric information. We will confront this with the
# computed models The SEDs show that upper limits are not very
# informative here, so we will *NOT* include information from them.
phot_info = asciitable.read(path_object + obj +'_processed.txt')
observed_detections =  phot_info['Detection']
indexes_detections = (observed_detections == 1)
observed_lmb = phot_info['Lmb[um]'][indexes_detections]
observed_lmbflmb = phot_info['Lmb_flmb[erg/cm2/s]'][indexes_detections]
observed_lmbflmb_errors = phot_info['Lmb_flmb_err[erg/cm2/s]'][indexes_detections]


# Read the fixed parameters for the model, and put them in a model
# Class.  We will pass this file, together with the object, to a
# function that will return the object already shaped.
parameters_objects = asciitable.read(path_main + 'parameters.csv', delimiter=';')
initial_model = shape_model(obj,parameters_objects)
